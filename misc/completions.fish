...
complete -c clifm -s 1 -l oneline -d 'List one file per line'
complete -c clifm -s a -l show-hidden -d 'Show hidden files' -x -a 'first last true false'
...
complete -c clifm -s i -l ignore-case -d 'Ignore case distinctions in filenames and paths'
complete -c clifm -s I -l no-ignore-case -d 'Do not ignore case distinctions'
complete -c clifm -s k -l keybindings-file -r -d 'Set an alternative keybindings file'
complete -c clifm -s l -l long-view -d 'Display extended file metadata'
complete -c clifm -s L -l dereference -d 'Follow symbolic links when listing files'
...
complete -c clifm -s z -l sort -r -d 'Set sort order' -x -a 'none name iname size atime btime ctime mtime version iversion extension iextension blocks links inode owner group type'
...
complete -c clifm -l color -d 'When to colorize ouput' -x -a 'always auto never'
...
complete -c clifm -l kitty-keys -d 'Ask the terminal to enable the kitty keyboard protocol'
complete -c clifm -l list-and-quit -d 'List files and quit'
complete -c clifm -l ls -d 'Same as --list-and-quit, but in a more ls-like style'
...
complete -c clifm -l names-last -d 'Print filenames after metadata (long view)'
...
complete -c clifm -l no-dereference -d 'Do not follow symblic links when listing files'
...
complete -c clifm -l no-file-counter -d 'Disable the file counter for directories'
...
complete -c clifm -l no-history -d 'Do not write commands into the history file'
complete -c clifm -l no-open-auto -d 'Same as no-cd-auto, but for files'
complete -c clifm -l no-refresh-on-resize -d 'Do not update the file list upon window\'s resize'
...
complete -c clifm -l no-truncate-names -d 'Do not truncate filenames'
complete -c clifm -l no-unicode -d 'Do not use Unicode decorations'
...
complete -c clifm -l physical-size -r -d 'Display physical file sizes (disk usage) rather than logical sizes (apparent size)'
...
complete -c clifm -l readonly -d 'Disable internal commands able to modify the filesystem'
complete -c clifm -l report-cwd -d 'Report the current directory to the terminal'
...
complete -c clifm -l sort-reverse -d 'Sort in reverse order'
...
complete -c clifm -l tabmode -r -d 'Set the tab completion mode' -x -a 'fzf fnf smenu standard'
complete -c clifm -l time-style -r -d 'Time/date style used in long view'
complete -c clifm -l total-size -d 'Display recursive directory sizes (long-view/--stat only)'
...
complete -c clifm -l virtual-dir-full-paths -d 'Files in virtual directories are listed as full paths instead of target basenames'
complete -c clifm -l virtual-dir -r -d 'Absolute path to a directory to be used as virtual directory'
complete -c clifm -l vt100 -d 'Run in VT100-compatible mode'
