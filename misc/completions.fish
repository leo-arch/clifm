...
complete -c clifm -s b -l bookmarks-file -r -d 'Specify an alternative bookmarks file' -x -a '(__fish_complete_path)'
complete -c clifm -s c -l config-file -r -d 'Specify an alternative configuration file' -x -a '(__fish_complete_path)'
complete -c clifm -s D -l config-dir -r -d 'Specify an alternative configuration directory' -x -a '(__fish_complete_directories)'
...
complete -c clifm -s k -l keybindings-file -r -d 'Specify an alternative keybindings file' -x -a '(__fish_complete_path)'
...
complete -c clifm -s p -l path -r -d 'Set starting path' -x -a '(__fish_complete_directories)'
complete -c clifm -s P -l profile -r -d 'Specify a profile to use' -x -a '(ls ~/.config/clifm/profiles/)'
...
complete -c clifm -s w -l workspace -r -d 'Specify the starting workspace'
...
complete -c clifm -s z -l sort -r -d 'Specify a sorting order' -x -a 'none name size atime btime ctime mtime version extension inode owner group'
...
complete -c clifm -l color-scheme -r -d 'Specify a color scheme' -x -a '(ls ~/.config/clifm/colors/*.clifm | awk -F\'/\' \'{print $NF}\' | cut -d\'.\' -f1)'
...
complete -c clifm -l max-files -r -d 'Specify a maximum number of listed files on screen'
...
complete -c clifm -l open -r -d 'Open FILE (via Lira) and exit' -a '(__fish_complete_path)'
complete -c clifm -l opener -r -d 'Use APP as resource opener'
complete -c clifm -l preview -r -d 'Display a preview of FILE (via shotgun) and exit' -a '(__fish_complete_path)'
...
complete -c clifm -l sel-file -r -d 'Specify a custom selections file' -a '(__fish_complete_path)'

complete -c clifm -l share-selbox -d 'Make the Selection Box common to different profiles'

complete -c clifm -l shotgun-file -r -d 'Specify a custom configuration file for shotgun' -a '(__fish_complete_path)'
...
complete -c clifm -l virtual-dir -r -d 'Absolute path to a directory to be used as virtual directory' -a '(__fish_complete_directories)'
