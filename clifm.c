 ...
 * not found, or if it's a wrong value, the default is set. */
{
	...
	if (!extcolors) {
		 ...
		 * when listing files */
		ext_colors_len = (size_t *)xnmalloc(ext_colors_n,
											sizeof(size_t));
 ...
 * "[date] msg", where 'date' is YYYY-MM-DDTHH:MM:SS */
{
	...
	if (!config_ok || !MSG_LOG_FILE || *MSG_LOG_FILE == '\0')
		return;
...
edit_bookmarks(char *cmd)
{
	...
	if (!cmd) {
		...
		if (opener) {
			...
			if (launch_execve(tmp_cmd, FOREGROUND)
			!= EXIT_SUCCESS) {
...
/* Log 'comm' into LOG_FILE */
{
	...
	if (!last_cmd) {
		 ...
		 * in the prompt function). However ... */
			...
			strcpy(last_cmd, _("Error getting command!\0"));
		}
	}
...
/*	if (!comm || *comm[0] == '\0')
	...
	|| strcmp(comm[0], "zz") == 0  || strcmp(comm[0], "salir") == 0
	|| strcmp(comm[0], "chau") == 0) {
