...
/*
###LICENSE NOTICE###
Copyright (c) 2018, L. Abramovich

	This program is free software; you can redistribute it and/or modify
	it under the terms of the GNU General Public License (version 2) as 
	published by the Free Software Foundation.

	CliFM is distributed in the hope that it will be useful, but WITHOUT 
...
/*
* DEPENDENCIES: 'glibc', 'ncurses', 'libcap', 'readline', 'coreutils' (providing basic programs 
* such as rm, cp, mkdir, etc). For Archers: all these deps are part of the 'core' repo, and 
* 'glibc' is also part of the 'base' group.
...
* Once you get the library, get its full path with 'find', and find which package owns that 
* file with pacman -Qo full_path
* OPTIONAL DEPENDENCIES: 'xdg-utils' and 'which' (without which xdg-open will fail to get the
* default application for files)
* */

/* COMPILING this program:
 * Do not forget to link the readline and cap libraries with the lreadline and lcap flags 
 * respectivelly:
 * $ gcc -lreadline -lcap -o clifm clifm.c
 * However, it's strongly recommended to optimize your program by adding these flags: -O2 and
 * -mtune=native. You could also try -O3 instead of -O2, which produces an even faster 
 * executable.
 * You can also protect your program from stack buffer overflow exploits by adding this flag:
 * -fstack-protector (-protection, +performance), -fstack-protector-strong, or 
 * -fsatck-protector-all (+protection, -performance). NOTE: Since 2014, Arch Linux packages 
 * are compiled with -fstack-protector-strong.
 * For debugging purposes, you can also add some debugging flags (optimization is not necessary here):
 * $ gcc -g -Wall -Wextra -Wstack-protector -pedantic -lreadline -lcap -o clifm clifm.c
 * Note: -g adds debugging symbols (readable by Valgrind and GDB, for example) to the code, but 
 * it makes the executable two or three times bigger.
 * When debugging it's no necessary anymore, simply remove all the debugging flags. The final 
 * compilation should not include warning and error flags at all, but only optimization and 
 * protection flags, plus readline and cap libraries:
 * $ gcc -O2 -mtune=native -fstack-protector-strong -lreadline -lcap -o clifm clifm.c
 */

//On code optimization see https://www.codeproject.com/Articles/6154/Writing-Efficient-C-and-C-Code-Optimization
...
'S_IFDIR' and the others cannot be used.*/
...
#include <unistd.h> //sleep, readlink, chdir, symlink, access, exec, isatty, setpgid, getpid
					//getuid, gethostname, tcsetpgrp, tcgetattr, char **__environ
...
//#include "clifm.h"
#include <fcntl.h> // O_RDONLY, O_DIRECTORY macros
...
#define CLEAR puts("\033c") //if no formatting, puts (or write) is faster than printf
//#define CLEAR write(STDOUT_FILENO, "\033c", 3)
#define COMM_FILE "/tmp/clifm/.clifm_comm"
#define VERSION "0.9.40.12-b (fm/shell version)"
#define AUTHOR "L. Abramovich"
#define CONTACT "johndoe.arch@outlook.com"
#define DATE "Sep 21, 2018"
#define WELCOME_MSG "Welcome to CliFM, the anti-eye-candy/KISS file manager!"

//Define flags for program options and internal use
static int flags; //variable to hold all the flags (int == 4 bytes == 32 bits == 32 flags)
//options flags
#define BACKUP_OK 		(1 << 0) //1 dec, 0x01 hex, 00000001 binary
#define COMM 			(1 << 1) //2 dec, 0x02 hex, 00000010 binary
#define FOLDERS_FIRST 	(1 << 2) //4 dec, 0x04 hex, 00000100 binary
#define HELP			(1 << 3) //and so on...
#define HIDDEN 			(1 << 4)
#define ON_THE_FLY 		(1 << 5)
#define SPLASH 			(1 << 6)
#define CASE_SENS 		(1 << 7)
#define START_PATH 		(1 << 8)
#define PRINT_VERSION	(1 << 9)
//internal flags
#define IS_PIPE			(1 << 10)
#define STREAM_REDIR	(1 << 11)
#define CONC_CMD		(1 << 12)
#define ROOT_USR		(1 << 13)
#define EXT_HELP		(1 << 14)
#define XDG_OPEN_OK		(1 << 15)
#define WINDOWED		(1 << 16)
#define IS_USRVAR_DEF	(1 << 17) //18 dec, 0x12 hex, 00010010 binary

//Replace strlen by my custom (faster, I think) xstrlen.
/*
#undef strlen
#ifndef strlen
...
#define strlen xstrlen //All calls to strlen will be replaced by a call to xstrlen
#define atoi xatoi
#define alphasort xalphasort
//#define strcmp(x,y) xstrcmp(x,y)

/* NOTE:
 * I've tried pthread to run some procedures in parallel, which is supposed to boost the
 ...
 * Most time-consuming functions according to gprof: get_path_programs (211 miliseconds per 
	call), get_history (112), bookmarks_function (96), open_function (85.88), list_dir (85.61)
	check_log_file (75), prompt (28.26), exec_prompt_cmds (27.99), and exec_cmd (27.54). Among
	them, those most called are: exec_cmd (32), exec_prompt_cmds (16), prompt (16), 
	dir_list (11), and open_function (5). get_path_programs and get_history, though the most 
	time-consuming by far, are called only once at the begining of the program. The most crucial
	function to be optimized is still dir_list. 
 * When testing some code performance, always do it outside Valgrind and optmimizing it via
	the compiler. What is slow in Valgrind, might be fast outside of it.

*/
//###TODO LIST###
/*
 ** There's a function for tilde expansion called wordexp().
 ** Only global variables are automatically initialized by the compiler; local variables must
	be initialized by the programer. Check there are no uninitialized local variables.
 ** The list-on-the-fly check is not elegant at all! It's spread all around the code. I'm not
	sure if there's an alternative to this: I have to prevent listdir() from running, and it
	is this function itself which is spread all around the code. I cannot make the on-the-fly 
	test inside the dirlist function itself, since the function is preceded by the free_dirlist 
	function, which frees the dirlist array to be filled again by listdir(). So, this freeing 
	must also be prevented, and thus, the check must be previous to the call to listdir().
 ** Set a limit, say 100, to how much directories will the back function recall.
 ...
 ** Replace "CLiFM" (only present in config files and help) by PROGRAM_NAME. 
 ...
 ** Add some keybinds, for instance, C-v to paste (see test3.c). Other posibilities: folders 
 ...
 ** When the string is initialized as NULL, there's no need for a calloc before the 
	reallocation.
 ** It seems that strlcpy (and family) is safer than strcpy (and family). On these functions
	see its manpage and https://www.sudo.ws/todd/papers/strlcpy.html. Whereas strcpy might 
	produce a buffer overflow (if dest is bigger than dest), strncpy will not NULL terminate 
	the string if dest does not contain a NULL char among the n specified bytes in strncpy. 
	Now, this will produce a segfault if, for example, we use strlen, which looks for a NULL 
	terminated byte. strlcpy, but contrast, adds a single null char at the end of the string 
	copied into dest. On the other side, strncpy will zero fill the remaining bytes of dest
	whenever src is shorter than dest, and this is a performance issue. Let's say we have 
	this code: 'strncpy(dest, src, 100)', and that src length is 50 bytes. In this case, 
	strncpy will copy the 50 bytes of src into dest, plus 50 more zeroes to reach 100 bytes. By
	contrast, strlcpy doesn't do this:  
 ...
 ** Take a look at the ouput of gcc When compiled with the following flags:
	(SOLVED) a) -Wformat-overflow=2: I get a couple of errors coming from get_properties(). 
	(SOLVED) b) -Wformat=2 in a couple of minor functions.
	(SOLVED) c) -Wformat-security= there are a couple of fprintf that do not specify the string format, 
	which could be a security whole.
	(SOLVED) d) -Wformat-truncation=2: some printf outputs may be truncated due to wrong sizes.
	(SOLVED) e) -Wformat-y2k: The strftime manpage says we don't need to worry about this warnings.
	It recommends indeed to use -Wno-format-y2k instead to avoid these warnings.
	f) -Wstrict-overflow=5
	g) -Wshadow
	(SOLVED) h) -Wunsafe-loop-optimizations
	i) -Wconversion: many warnings
 ...
 ** 17 - Be careful when removing symlinks. I remove them with 'rm -r', but perhaps I should 
	...
	symlink.
 ** 18 - Scalability/stress testing: Make a hard test of the program: big files handling, large 
	amount of files, heavy load/usage, etc. 
 ** 19 - Destructive testing: Try running wrong commands, MAKE IT FAIL! Otherwise, it will never 
	leave the beta state.
 ** 20 - Add a trash function.

 * (DONE) I'm using more than 15 calls to printf to print the splash screen. What a waste! 
	Reduce them to just one.
 * (DONE) Re-write the alphasort_insensitive function using pointers, to get rid of the leading
	dot of hidden files, and strcasecmp for a case insensitive comparison. In this way, the
	function is twice faster than before (specially when big amount of files are involved)!!
 * (DONE) Write a custom alphasort function using strcmp, instead of strcoll, used by the built-in
	alphasort, to compare dir names. Why? Simply because strcmp is faster than strcoll.
 * (DONE) There are two unused functions: str_ends_with, and strbfrlst. Comment them out.
 * (DONE) Try a custom and faster implementation of atoi (say, xatoi) and rewrite digits_in_num 
	to make it faster. Both functions are called by list_dir.
 * (DONE) Remove the call to lstat from both folder and file_select functions and replace it
	by a simple check for entry->dt_type == DT_DIR. This data is provided by the own scandir 
	function. 'lstat' simply adds an extra and unnecesary function.  
 * (DONE) /tmp is being accessed twice for each command line input, once to check for
	aliases and once to run prompt cmds. I guess this could be slowing things down. I should 
	try storing both aliases and prompt cmds into two arrays of strings at the beginning of
	the program. Rewrite the get_aliases_n_prompt_cmds function to use global variables 
	instead of tmp files to store this information. Result: whereas there's seems to be no 
	change int the prompt function speed, the check_for_alias function is now almost 6 times
	faster. However, the whole program takes now 100k aprox of memory more than before. But, 
	it's still below the 5Mb, 4.6 indeed.
 * (DONE) User defined variables only works for the CLiFM shell. Make it work for the system
	shell as well, it's a nice feature.
 * (DONE) Get rid of the DEFAULT_PATH macro (it gets the user's home dir). It calls two 
	functions (getpwuid and getuid) every time it is called, being thus unnecessarily expensive.
	Just run these functions once at the beginning and store the user's home dir into a dynamic 
	array.
 * (UNNEEDED) It could be a good idea to add a puts call in every function to track when, and 
	how many times functions are called. Just use gprof to profile the program! It does 
	exactely this.
 * (DONE) Let folders-first, show-hidden, and list-on-the-fly as default. Add command line
	options to enable AND to disable these features.
 * (DONE) Rewrite the count_dir function using getdents instead of scandir or readdir. 
	Getdents is 4 times faster than scandir and almost 2 times faster than readdir. However,
	It FAILS when trying to access a shared mount point. Going back to the readdir version.
 * (DONE) According to gprof, the most used function is by far alphasort_insensitive. So, this 
	is a perfect place for code optimization. Replace tolower() by some custom code and rewrite 
	the loops to make them decrease instead of increase.  
 * (DONE) Add an explanation of the history command (!) to help.
 * (DONE) - Make the 'back' function remember more paths. It only goes back once. 'old_pwd' 
	(directory history recollection) is a FIXED array: it always takes 4096 
	bytes!, even if the current value takes only 20 bytes. Even more, it should be an ARRAY of 
	strings containing ALL visited directories (and not just the last one). I added some 
	subfunctions (like clear, hist and ELN) to the main 'back' function. I added this function 
	to the bookmarks function as well.
 * (DONE) Find a way to conveniently handle realloc and calloc errors (if possible, not exiting the
	program). I wrote custom functions to do this: xcalloc() and xrealloc(), replacing every
	call to malloc/calloc and realloc by xcalloc and xrealloc respectivelly.
 ...
 * (DONE) Remove strlen from loops. in 'i=0;i<strlen(str);i++' strlen is called as many times 
	as chars contained in str. To avoid this waste of resources, write the loop as this:
 ...
 * (DONE) Change the way of listing files in list_dir(): use the d_type field of the dirent
	...
	(lstat). By doing this I save one or two commands for EACH listed file, which is a lot.
...
###KNOWN BUGS###
 ** 1 - BACKUP doesn't work as it should when files are removed using wildcards.
 ** 2 - Since removed files are copied into /tmp, whenever big files, or a great amount of 
	files are removed, the capacity of /tmp could be easily exceeded producing thereby errors, 
	like running out of memory!
 ** 3 - If rm makes a question regarding the file to be removed, this question won't be seen 
	on the screen (I guess the same happens with cp and mv, at least). The problem is 
	check_cmd_stderr().
 ** 4 - Path highlighting in the back hist function fails when path starts with tilde (~).
 ** 5 - Specify a file (not a dir) as destiny of the 'paste sel' command, and 
 ++ CRASH!!! 
 ** 6 - This command: "sel /usr/local/sbin/*" doesn't work, for it produces 
 ++ these wrong paths: "/home/leo//usr/local/sbin/{filenames}". It only happens
 ++ when using the wildcard.

 * (SOLVED) Every time the screen is refreshed by some keyboard shortcut (C-f, C-h, or C-r), 
	the first entries of the files list appear in the prompt line. Solution: add a puts("")
	before calling list_dir().
 * (SOLVED) Select some files, go to the deselect screen and then type "*". The program crashes.
	Problem: my xatoi returns decimal values for alphabetical chars. Solution: add a conditional
	statement to check for this chars.
 * (SOLVED) Type "slaby", press Enter, and bang!, the program crashes. Problem was: I'm not
	sure. Perhaps it only happens when using the CLiFM shell.
 * (SOLVED) When not listing files automatically, the bookmarks function doesn't update the 
	prompt. Solved by simply adding a 'chdir(path)' in case of no list-on-the-fly.
 * (SOLVED) There are duplicated highlighted lines in the back hist function whenever there 
	are two or more lines pointing to the same path. Problem: wrong condition for highlighting 
	lines.
 * (SOLVED) When running as root, the program cannot be called as argument for a terminal. See 
	the setpgid lines in init_shell(). I'm not sure why, but this doesn't happen anymore.
 * (SOLVED) Wrongly listing non-empty files as empty. Check /proc/pid/cmdline. Stat's st_size is not
	accurate enough. Not my problem; the buil-in 'stat' command has the same issue, if it's
	an issue at all.
 * (SOLVED) Errors in Valgrind when: 1 - search for a pattern; 2 - select an element; 3 - 
	search for the first pattern again. I guess this bug was fixed when the search function was
	rewritten.
 * (SOLVED) Weird: when opening ~/.xinitrc as 'o(pen) ELN/filename' it doesn't remain in the 
	foreground. It only happens with THIS file. Not a problem of my program, it happens as well 
	in other terminals. It seems to be a problem of xdg-open: it doesn't happen when using 
	leafpad for example.
 ...
 * (SOLVED) 'ls' stopped working. Problem: the '=' case of the switch statement in 
	...
	SOLUTION: check for spaces before '=' in order to avoid taking as variables things like:
	'ls -color=auto'.
 ...
 * (SOLVED) When it comes to optical drives (CD-ROM) files color is always default, i.e., its
	...
	be scandir's st_type. the stat function instead doesn't have this problem. So, whenever 
 ...
 * (SOLVED) The user can run any shell whithin the program, for instance, 'exec bash'. SOLUTION: 
	Not a problem.
...
//2 times faster than atoi. The commented lines make xatoi able to handle negative values.
{
//	if ()
	int ret=0;//, neg=0;
/*	if (*str == '-') { // same as: str[0] == '-'
	...
	for(size_t i=0; i<len; i++) {
		if (str[i] < 0x30 || str[i] > 0x39) return ret;
		ret=ret*10+(str[i]-'0');
	}
...
/*
int xstrcmp(const char *str_a, const char *str_b)
{
	size_t str_len_a=xstrlen(str_a);
	size_t str_len_b=xstrlen(str_b);
	if (str_len_a > str_len_b) return 1;
	else if (str_len_a < str_len_b) return -1;
	//At this point, both strings have the same length, so it's the same to use str_len_a or
	//str_len_b
	for (size_t i=0;i<str_len_a;i++) {
		if (str_a[i] > str_b[i])
			return 1;
		else if (str_a[i] < str_b[i])
			return -1;
	}
	//Only reached if all chars in both strings are the same (in the same order)
	return 0;
}
*/
...
/*
	...
	printf("Shell: %s\n", user_info->pw_shell);
*/
	size_t str_len=strlen(user_info->pw_dir);
	char *p=calloc(str_len+1, sizeof(char)), *home=NULL;
	if (p) {
		home=p;
		p=NULL;
		strncpy(home, user_info->pw_dir, str_len+1);
		return home;
	}
	else
		return NULL;
...
{	
/*	for (size_t i=0;i<strlen(str);i++) { //Worst version: 2 functions each iteration + 1
		if (!(isdigit(str[i])))			 //extra variable
			return 0;
	}
	return 1;

	for (size_t i=0;str[i];i++) //###VERSION 1: isdigit + one variable
		if (!(isdigit(string[i])))
	
	for (size_t i=strlen(str);i--;) //###VERSION 2: strlen + one variable
		if (str[i] < 0x30 || str[i] > 0x39)
			return 0;
	return 1;
*/
	while(*str) { //###VERSION 3: NO isdigit, NO strlen, and NO extra variables at all!
...
//	return snprintf(NULL, 0, "%d", num)-(num < 0); //VERSION 1
...
//Same a alphasort, but is uses strcmp instead of sctroll, which is slower.
{
	/* I'm not sure whether these if statements make any difference. They prevent strcmp 
	from running in every call to the function, which is supposed to make the function
	faster */
...
{
	return strcasecmp(((*a)->d_name[0] == '.') ? (*a)->d_name+1 : (*a)->d_name, 
						((*b)->d_name[0] == '.') ? (*b)->d_name+1 : (*b)->d_name);
}

char *handle_spaces(char *str)
/*Remove leading, terminating, and double spaces from str. Returns NULL if str: a) is NULL;
b) contains only spaces; c) is empty; d) there was a memory allocation failure*/
{
	if (!str || strlen(str) == 0) return NULL; //if str is NULL or zero length
	char *f_str=NULL;
	int first_non_space=0, counter=0;
	//if I'd use while(*str), I'd have to increment str at the end of the loop
	for(;*str;str++) {
		if (*str != 0x20)
			first_non_space=1;
		if (first_non_space) {
			if (*str != 0x20 || (*str == 0x20 && *(str+1) != 0x20 && *(str+1) != 0x00)) {
				counter++;
...
/* Returns the index of c in str, if any, and -1 if c is not found. */
{
//	size_t str_len=strlen(str);
//	for (size_t i=str_len;i--;) {
	int counter=0;
...
/*returns the string after the first appearance of a given char, or returns NULL if c is not 
found in str, or c is the last char in str. */
{
	char *f_str=NULL;
	int counter=0;
	size_t str_len=strlen(str);
	while(*(str)) {
		if ((str_len-1) == counter) return NULL; //There's no string after last char
		if (*str == c) {
...
/*Returns the substring in str before the first appearance of c. If not found, or c is the 
first char in str, returns NULL. */
{
	char *start=str, *f_str=NULL;
	int counter=0;
	while(*str) {
		if (*str == c) {
			if (counter == 0) return NULL; //There's no substring before the first char
			...
			if (p) {
				...
				return f_str;
				/*NOTE: f_str is null terminated because calloc allocated and zeroed count+1
				bytes, whereas strcpy copied the first 'counter' bytes of start into f_str. */
			}
			else return NULL;
		}
		counter++;
...
//returns the string in *str before the last appearance of a given char (c). If not found, 
...
//the first or the last in the string.
{
	...
	for (unsigned i=str_len;(int)i>=0;i--) {
		if (str[i] == c) {
			...
			char *p=calloc(index+1, sizeof(char)), *f_str=NULL;
			if (p) {
				f_str=p;
				p=NULL;
			}
...
/*Returns the string in 'str' between chars 'a' and 'b'. If not found, returns NULL. 
...
/*
char *strbtw_new(char *str, char a, char b)
//I get errors from this function when compiling like this: -O3 -mtune=native 
//-fstack-protector-strong 
{
	char *f_str=NULL, *start=NULL;
	int start_ok=0, counter=0;
	while (*str) {
		if (!start_ok && *str == a) {
			start=str+1;
			start_ok=1;
		...
		if (start_ok) {
			if (*str == b) {
				char *p=calloc(counter, sizeof(char));
				if (p) {
					f_str=p;
					p=NULL;
					strncpy(f_str, start, counter-1);
					start=NULL;
					return f_str;
				}
				else return NULL;
...
/*Get all substrings from str using IFS as substring separator*/
{
	if (!str) return NULL;
	char **substr=NULL;
	char buf[1024]="";
...
char *get_date(void)
{
	...
	char *p=calloc(64, sizeof(char)), *date;
	if (p) {
		date=p;
		p=NULL;
	}
...
//	#endif
	FILE *fp;
	char proc_file[24]="";
	char *p=calloc(MAX_LINE, sizeof(char)), *line;
	if (p) {
		line=p;
		p=NULL;
	}
...
void add_to_dirhist(char *dir_path);
...
The STACK is used to store FIXED variables that are used only INSIDE a function (LOCAL). 
Stack memory is temporary. Once a function finishes running, all of the memory for its 
variables is freed, unless the variable was declared as static, in which case it's stored in
the data segment.
The HEAP is where dynamic variables are stored. Everything going through malloc, realloc, and
free goes to the heap.

The STACK is MUCH MORE FASTER THAN THE HEAP, THOUGH DINAMYCALLY ALLOCATED ARRAYS GENERALLY 
MAKE THE PROGRAM MUCH MORE LEIGHTWEIGHT IN TERMS OF MEMORY FOOTPRINT. Ex: 
...
You ca use the 'size' command to see the size of the text, data, and bbs segments of your 
program. Ex: $ size clifm
...
INT: 2 (for old 16 bits PC's) or 4 bytes (32 bits). if 4 bytes: -2,147,483,648 to 
	2,147,483,647 (if signed). 
...
POINTERS: Ex: char *p; pointers, unlike common variables which stores data, store memory 
addresses, which in 32 bits machines take 4 bytes (0xffff3d98) and in 64 bits machines 8 bytes
(0xffffffffd989ed4f). So, while 'char p' takes only 1 byte, 'char *p' takes 4/8 bytes. Since
pointers store only memory addresses, the type of pointer (char, int, float, etc.) has nothing
to do with its length; it's always 4/8 bytes. NOTE: in 16 bits machines a pointer size is 2
bytes.

Note: unsigned data types may contain more positive values that its signed counterpart, but at
the price of not being able to store negative values. size_t is just an alias for unsigned int.
See: http://www.geeksforgeeks.org/memory-layout-of-c-program/

Garbage Collection (automatic memory handling) in C
Though C is designed to manually handle memory, there is a garbarge collector (gc) available 
(The Boehm-Demers-Weiser GC Library, or the WDW library) to automatically handle memory, much 
like Java or Bash does: just add the gc/gc.h library and then use the GC_malloc and GC_realloc 
macros instead of malloc/calloc and realloc. There is no need to call free(). In order to 
replace all calls to malloc/calloc and realloc by their corresponing GC functions, add the 
following define lines:
...
NOTE: The above are called macros. The macro name is replaced by its definition all along the
source code by the pre-processor even before compilation. If we have this macro:
#define MAX 10
...
//###GLOBAL VARIABLES####
/* These variables, unlike local or automatic variables, are accessible to any part of the 
 program and hold their values throughout the entire lifetime of the program. If not 
 initialized, they're initialized automatically by the compiler in the following way:
 int 		0
 char 		'\0'
 float	 	0
 double 	0
...
Local variables are only accesible from within the function they were declared, and loses their
once the function ended. The same goes to for the block variables, only that these variables 
only work within the block in which they were declared. Example: if (x) int a=1; the variable
'a' will only be visible within the if block. Parameters passed to a function are also 
...
On why to reduce variables scope as much as possible see: https://refactoring.com/catalog/reduceScopeOfVariable.html

When initializing a char string, make sure you add the NULL terminator or leave room, 1 byte, 
so that it can be added automatically. Example: char [6]="hello\0", or char[6]="hello", or
char[]="hello". Do not initialize it like this: char [5]="hello", for this will be a 
...
int files=0, args_n=0, sel_n=0, max_hist=-1, max_log=-1, pipes_index=0, path_n=0, 
	current_hist_n=0, bg_proc_n=0, dirhist_total_index=0, dirhist_cur_index=0, argc_bk=0, 
	usrvar_n=0, aliases_n=0, prompt_cmds_n=0; 
	//current amount of user defined variables; //-1 means non-initialized
size_t user_home_len=0;
struct termios shell_tmodes;
pid_t bg_proc[100]={ 0 }, own_pid=0;
struct dirent **dirlist=NULL;
struct usrvar_t *usr_var=NULL;
char *user=NULL, *path=NULL, **old_pwd=NULL, **sel_elements=NULL, sel_file_user[64]="", 
	**paths=NULL, **bin_commands=NULL, **history=NULL, *xdg_open_path=NULL, **braces=NULL, 
	prompt_color[19]="", **prompt_cmds=NULL, **aliases=NULL, **pipes=NULL, **argv_bk=NULL, 
	*user_home=NULL; //**my_envp=NULL, *my_invocation_name=NULL,
char CONFIG_DIR[CONF_PATHS_MAX]="", CONFIG_FILE[CONF_PATHS_MAX]="", BM_FILE[CONF_PATHS_MAX]="", 
	 ...
	 BK_DIR[CONF_PATHS_MAX]="", PROFILE_FILE[CONF_PATHS_MAX]="", 
	 *INTERNAL_CMDS[27]={ "alias", "open", "prop", "pr", "back", 
...
int main(int argc, char **argv)
{
	...
	if ((user_home=get_user_home()) == NULL) { //get user's home directory
		user=xcalloc(user_home, 4, sizeof(char *));
		strncpy(user_home, "???\0", 4);
	}
...
//	getcwd(ORIGINAL_PWD, sizeof(ORIGINAL_PWD));
	...
	if (gethostname(hostname, sizeof(hostname)) == -1)
		strncpy(hostname, "???\0", 4);
	path=xcalloc(path, user_home_len+1, sizeof(char));
	strcpy(path, user_home); //set initial path to default path, that is, user's home
//	strncpy(old_pwd, path, strlen(path));
	...
	'1' is just the program invokation name. External arguments will override initialization 
	values (init_config).*/
	...
	readline_kbinds(); //Initialize the keyboard bindings function
	
	//###MAIN PROGRAM LOOP####
	/* This is the structure of a basic shell 
	1 - Inifnite loop
	2 - Grab user input
	3 - Parse user input
	4 - Execute command
	*/
	while (1) { //or: for (;;) --> Infinite loop to keep the program running
...
void add_to_dirhist(char *dir_path)
//Add new path to visited directory history
{
	//Do not add anything if new path == last entry in directory history
	...
	old_pwd[dirhist_total_index]=xcalloc(old_pwd[dirhist_total_index], strlen(dir_path), sizeof(char *));
...
/*	rl_bind_keyseq("\\C-a", readline_kbind_action); //key: 1
	...
	rl_bind_keyseq("\\C-c", readline_kbind_action); //key: Doesn't work
	rl_bind_keyseq("\\C-d", readline_kbind_action); //key: Doesn't work
	...
	rl_bind_keyseq("\\C-p", readline_kbind_action); //key:
	rl_bind_keyseq("\\C-q", readline_kbind_action); //key:
	rl_bind_keyseq("\\C-r", readline_kbind_action); //key: 18
	rl_bind_keyseq("\\C-s", readline_kbind_action); //key: FREEZES!!
	NOTE: C-s blocks the terminal. It's a feature of the OS called 
	Software Flow Control (XON/XOFF flow control). To reenable it simply press C-q.
	rl_bind_keyseq("\\C-t", readline_kbind_action); //key: 20
	rl_bind_keyseq("\\C-u", readline_kbind_action); //key: Doesn't work
	rl_bind_keyseq("\\C-v", readline_kbind_action); //key: Doesn't work
	rl_bind_keyseq("\\C-w", readline_kbind_action); //key: Doesn't work
	rl_bind_keyseq("\\C-x", readline_kbind_action); //key: 24
	rl_bind_keyseq("\\C-y", readline_kbind_action); //key: 25
	rl_bind_keyseq("\\C-z", readline_kbind_action); //key: Doesn't work
...
//				free_dirlist();
				puts(""); //without this, the first two entries are printed in the prompt line
				list_dir();
			}
			break;
		case 8: //C-h
...
//			free_dirlist();
		...
		case 25: //C-y
			...
			if (status !=sys_shell_status) {
				puts(""); //without this, the first prompt command is printed in the prompt line
				char *input=prompt();
				free(input);
			}
			break;
...
/*
void free_usrvars(void)
//Since this little function is only used once by the free_stuff function, it makes no sense 
...
//	free_dirlist();
	...
	if (dirhist_total_index) {
		for (int i=0;i<dirhist_total_index;i++) free(old_pwd[i]);
		free(old_pwd);
	}
	for (unsigned i=0;(int)i<usrvar_n;i++) {
		free(usr_var[i].name);
	...
	for (size_t i=0;i<(size_t)prompt_cmds_n;i++) free(prompt_cmds[i]);
	free(prompt_cmds);
/*	
	if (my_envp) {
...
char *bin_cmd_generator(const char *text, int state)
{
	...
	if (!state) {
		i=0;
		len=strlen(text);
	} /*state is zero only the first time the function is called, and a non-zero positive in
	later calls. This means that i and len will be necessarilly initialized the first time.*/
...
//send all paths of PATH environment variable to a globally declared array (paths)
{
	...
	for (i=0;path_tmp[i];i++) {
		...
		paths[path_num]=xcalloc(paths[path_num], strlen(path_tmp)+1, sizeof(char *)); //should be PATH_MAX
		int length=0;
		while (path_tmp[i] && path_tmp[i]!=':') {
			paths[path_num][length++]=path_tmp[i++];
		}
		path_num++;
	}
	free(path_tmp);
...
//set up CliFM directories and config files. Load the user's configuration from clifmrc
{
	...
	snprintf(CONFIG_DIR, user_home_len+15, "%s/.config/clifm", user_home);
...
//	snprintf(PROMPT_FILE, tmp_dir_len+user_len+21, "%s/.clifm_prompt_cmds_%s", TMP_DIR, user);
	...
	snprintf(sel_file_user, tmp_dir_len+13+user_len, "%s/.clifm_sel_%s", TMP_DIR, user);
	...
	if (stat(CONFIG_FILE, &file_attrib) == -1) { //if file doesn't exist, create it.
		...
		if (config_fp == NULL) {
			...
			fprintf(config_fp, "Show hidden files=true\n");
			fprintf(config_fp, "Backup deleted files=false\n");
			fprintf(config_fp, "List folders first=true\n");
			fprintf(config_fp, "cd lists automatically=true\n");
...
same as the 'which' built-in Linux command*/
{
	char *cmd_path=NULL;
...
//Link long (--option) and short options (-o) for the getopt_long function
	opterr=optind=0; /*disable automatic error messages to be able to handle them myself via 
	...
	static struct option longopts[]={
		...
		{"no-list-on-the-fly", no_argument, 0, 'l'},
	...
	while ((optc=getopt_long(argc, argv, "+aAbBc:CfFhiIlLp:Svx", longopts, (int *)0)) != EOF) {
		/*':' and '::' in the short options string means 'required' and 'optional argument'
		respectivelly. Thus, 'c' and 'p' require an argument here. The plus char (+) tells 
		getopt to stop processing at the first non-option*/
		switch (optc) {
			case 'a':
...
//Replace "/home/user" by "~/".
{
	...
	//if path != home dir...
	if (strncmp(path, user_home, path_len) != 0) {
		char *str_aft=NULL;
		str_aft=xcalloc(str_aft, path_len, sizeof(char *));
		int j=0;
		for (unsigned i=user_home_len+1;i<path_len;i++)
			str_aft[j++]=path[i];
...
A-Z), numbers (0-9), period (.), dash (-), and underscore ( _ ).*/
{
	...
	c) is empty; d) memory allocation error*/
	char *string_b=handle_spaces(str); //Remove isspace from here too.
	//NOTE: isspace() not only checks for space, but also for new line, carriage return, vertical
	//and horizontal TAB. Be careful when replacing this function.
	if (!string_b) return NULL;
	...
	/*if concatenation, pipes, stream redirection, or user variable creation, just return the 
	whole string to be parsed by the corresponding function*/
	unsigned i=0;
...
//						if (isspace(string_b[j])) space_found=1;
						if (string_b[j] == 0x20) space_found=1;
					}
...
//				}
	...
	//#### 3 - CHECK QUOTES ####
	//If neither null string nor pipe nor redirection nor contatenation nor user var...
	unsigned length=0; //unsigned == unsigned int == size_t
	char buf[PATH_MAX]="";
	int braces_index=-1;
	for (i=0;i<=str_len;i++) {
		...
		if (string_b[i] == '"' || string_b[i] == '\'') {
			...
			char quotes;
			if (string_b[i] == '"') quotes='"'; //hex value 0x22
			else quotes='\''; //hex value 0x27
			unsigned j;
		...
		*/
		...
		//If no quotes, check for backslashes to deal with filenames containing spaces
//		else if (string_b[i] == '\\' && isspace(string_b[i+1]))
		//NOTE: Why to call a function (isspace), if a simple comparison can do it?? Btw, 0x20
		//is the hex value for space.
		else if (string_b[i] == '\\' && string_b[i+1] == 0x20)
			buf[length++]=string_b[++i];
		
		//If neither space nor null, and the buffer is not full, dump the char into the buffer.
...
//		else if (!(isspace(string_b[i])) && string_b[i] != '\0' && (unsigned)length < sizeof(buf)) {
		else if (string_b[i] != 0x20 && string_b[i] != '\0' && (unsigned)length < sizeof(buf)) {
			buf[length++]=string_b[i];
...
//		else if (string_b[i] == '\0' || (isspace(string_b[i]) && !(isspace(string_b[i+1])))) { //&& string_b[i+1] != '\0') {
		else if (string_b[i] == '\0' || (string_b[i] == 0x20 && string_b[i+1] != 0x20)) { //&& string_b[i+1] != '\0') {
			...
			memset(buf, 0x00, length); //clean the buffer (0x00 == \0 == null char)
//			for (size_t i=0;i<length;i++) buf[i]='\0'; //clean the buffer
//			size_t p=length; while(p--) buf[p]=0; //clean the buffer
	...
	//#### 5 - NULL TERMINATE THE INPUT STRING ####
	//Add a terminating NULL string to the array
	comm_array=xrealloc(comm_array, sizeof(char **)*(args_n+2));	
	comm_array[args_n+1]=NULL;

	//#### 6 - CHECK FOR SPECIAL SIGNS ####
	//This should go in a different function (or perhaps not: it's used only once)
	for (i=0;i<=(unsigned)args_n;i++) { //replace "~/" by "/home/user"
		if (strncmp(comm_array[i], "~/", 2) == 0) {
			...
			if (path_no_tilde) {
				...
				comm_array[i]=xrealloc(comm_array[i], (user_home_len+2)*sizeof(char *));
...
void exec_prompt_cmds(void)
{
	if (prompt_cmds_n == 0) return;
	for (size_t i=0;i<(size_t)prompt_cmds_n;i++) {
		args_n=0;
		char **cmds=parse_input_str(prompt_cmds[i]);
		if (cmds) {
			no_log=1;
			exec_cmd(cmds);
			no_log=0;
...
//														   default_color, white, NC, default_color, NC);
		printf("%s%s%s\n%sType '%shelp%s%s' or '%s?%s%s' for instructions.%s\n", 
			magenta, WELCOME_MSG, NC, default_color, white, NC, default_color, white, NC, 
			default_color, NC);
		welcome_message=0;
	}
	exec_prompt_cmds();
...
//	size_t p=prompt_length; while(p--) shell_prompt[p]=0; //initialize shell_prompt
...
/*void free_dirlist() //Should I turn this into a macro? No advantage.
Function calls are expensive, in the best zero cost, but to be sure it will not be faster than
the code it runs running without the function call. So, do not use functions to do this kind
of little things. Simply do the little thing instead of calling of function to do it.
On functions cost see: https://mortoray.com/2010/10/12/the-cost-a-function-call/
{
	while (files--)
		free(dirlist[files]);
}*/
...
//	clock_t start=clock();
	...
	if (path[strlen_path-1] == '/' && strcmp(path, "/") != 0) //remove final slash from path, if any.
	...
	if (list_folders_first) {
		...
		if (files_folders == -1) {
			if (errno == ENOMEM) {
				fprintf(stderr, "%s: %s\n", PROGRAM_NAME, strerror(errno));
				atexit(free_stuff); //exit the program only after running the clean function
			}
			else {
				fprintf(stderr, "%s: %s\n", PROGRAM_NAME, strerror(errno));
				return;
		...
		//Reallocate the dirlist struct according to the amount of folders and files found
...
//			size_t str_len=0;
			for(i=0;i<files_folders;i++) {
				size_t str_len=strlen(dirlist_folders[i]->d_name);
				dirlist[files]=xcalloc(dirlist[files], str_len+1, sizeof(char *));
				/*1- Every element stored in dirlist[files] will be a char pointer, that's why
				I use sizeof(char *) instead of sizeof(char). The sizeof operator returns the
				the size of the type passed as arg, in this case 'char *'. Strictly speaking,
				I could also use here 'int *' or 'float *', and even 'char **', since all 
				pointer types equal to 8 (a 64 bits memory address, like 0xffffff6d346fad). 
				Note: 32 bits memory addresses are 4 bytes long instead of 8. 
				However, for code legibility I choose 'char *'.
				2- On the other side, I use 'str_len +1' to make room for the NULL terminator, 
				not counted by strlen.*/
				strncpy(dirlist[files++]->d_name, dirlist_folders[i]->d_name, str_len+1);
				/*If I use here only str_len as the max length of dest, the NULL terminator 
				(str_len+1) will not be copied into dest, and strncpy doesn't do that by itself.*/
//				dirlist[files++]->d_name[str_len+1]='\0';
...
//			size_t str_len=0;
			for(i=0;i<files_files;i++) {
				size_t str_len=strlen(dirlist_files[i]->d_name);
				dirlist[files]=xcalloc(dirlist[files], str_len+4, sizeof(char *));
				strncpy(dirlist[files++]->d_name, dirlist_files[i]->d_name, str_len);
		...
		//Completely free the dirlist struct, since it will be (re)allocated by scandir
		while (files--) free(dirlist[files]);
//		free_dirlist();
		free(dirlist);
		if ((files=scandir(path, &dirlist, skip_implied_dot, (case_sensitive) ? 
...
//	for(i=0;i<files;i++) {
	for (i=files;i--;) {
		...
		/* file_name_width contains: ELN's number of digits + one space between ELN and file 
		name + filename length. Ex: '12 name' contains 7 chars*/
	...
	short term_cols=w.ws_col; //w.ws_row is also available
...
//				else if (cap) {
					...
					printf("%s%d %s%s%s%s", yellow, i+1, bg_red_fg_black, dirlist[i]->d_name, NC, 
																  (last_column) ? "\n" : "");				
					cap_free(cap);
				}
				else if (file_attrib.st_mode & S_IXUSR)
		...
		if (!last_column) {
			//get the difference between the length of longest and the current element
...
//Copy aliases found in clifmrc to a tmp file: /tmp/.clifm_aliases
{
	...
	while (fgets(line, sizeof(line), config_file_fp)) {
		if (strncmp(line, "alias", 5) == 0) {
			...
			aliases[aliases_n]=xcalloc(aliases[aliases_n], 256, sizeof(char *));
			...
			if (strncmp(line, "#", 1) != 0) {
				prompt_cmds=xrealloc(prompt_cmds, sizeof(char **)*(prompt_cmds_n+1));
				prompt_cmds[prompt_cmds_n]=xcalloc(prompt_cmds[prompt_cmds_n], 256, sizeof(char *));
...
matching alias is found, zero if not.*/
{
	...
	for (size_t i=0;i<aliases_n;i++) {
		if (strncmp(aliases[i], comm_tmp, comm_len+1) == 0) {
			aliased_cmd=strbtw(aliases[i], '\'', '\''); //get the aliased command
...
void stream_redirection(char *string)
{
	flags &= ~STREAM_REDIR; //disable the stream redirection flag
	int override=1;
...
*/
	...
	if (strcmp(comm[0], "o") == 0 || strcmp(comm[0], "cd") == 0 
		...
		if (comm[1]) {
			if (strcmp(comm[1], "hist") == 0 || strcmp(comm[1], "h") == 0)
			...
			else if (is_number(comm[1]) == 1) {
				//Go the the specified directory (first arg) in the directory history list 
				if (atoi(comm[1]) > 0 && atoi(comm[1]) <= dirhist_total_index) {
					update_path(old_pwd[atoi(comm[1])-1]); 
					if (cd_lists_on_the_fly == 0) return;
					while (files--) free(dirlist[files]);
//					free_dirlist();
					list_dir();
				}
				else fprintf(stderr, "back: %d: No such element\n", atoi(comm[1]));
			}
			else fputs("back: Usage: b [hist] [clear] [ELN]\n", stderr);
			return;
		}
		//If first path in current dirhist was reached, make it equal to the total index
		if (dirhist_cur_index <= 0) {
			dirhist_cur_index=dirhist_total_index;
		}
		if (access(old_pwd[dirhist_cur_index-1], F_OK) == 0) {
			update_path(old_pwd[--dirhist_cur_index]); 
			if (cd_lists_on_the_fly == 0) return;
			while (files--) free(dirlist[files]);
//			free_dirlist();
			list_dir();
		}
		else fprintf(stderr, "%s: %s: %s\n", PROGRAM_NAME, old_pwd[dirhist_cur_index-1], strerror(errno));
	}
...
//		free_dirlist();
		...
		if (comm[n]) {
			...
			if (strcmp(comm[n], "on") == 0) list_folders_first=1;
				...
				fputs("Usage: folders first [on/off]\n", stderr);
				return;			
			}
...
//				free_dirlist();
		...
		if (comm[1]) {
			...
			else fputs("Usage: sys [on/off/status]\n", stderr);
		}
		else fputs("Usage: sys [on/off/status]\n", stderr);
	}
			...
			for (int i=1;i<=args_n;i++) {
				if ((strcmp(comm[0], "kill") == 0 && atoi(comm[i]) == get_own_pid()) || 
				 ((strcmp(comm[0], "killall") == 0 || strcmp(comm[0], "pkill") == 0) && 
						...
						fprintf(stderr, "%s: To exit the program type 'quit'\n", PROGRAM_NAME);
						return;
				}
			}
		}
...
//		####SYSTEM VERSION#####
		if (sys_shell) {
			if (comm[0][0] == ';') { //Tell CLiFM to run the command via the system shell
...
/*
		###No needed, since execvp() (in launch_execv()) handles this by itself###
...
//					free_dirlist();
				...
				if (strcntchr(comm[1], '*') != -1 || strcntchr(comm[1], '?') != -1) {
					...
					if (destiny != NULL) {
						add_to_dirhist(path);
						strncat(path, "/", 2);
						/*The bound is here set to 2, while the size of the source (/) is 1, 
						since str(n)cat will add a terminating null byte. That is, the bound 
						size should always be (at least) source+1.*/
...
//			int root_path=0;
		...
		//#####ELN#####
			...
			(size_t)digits_in_str(comm[i]) == strlen(comm[i])-1)) {
			...
			if (strcntchr(comm[i], '-') != -1) {
				...
				if (range_start && range_end) {
					if (is_number(range_start) && is_number(range_end)) {
						...
						int a_range_end=atoi(range_end);
						for (int j=atoi(range_start)-1;j<a_range_end;j++) {
...
//		free_dirlist();
	...
	for (i=0;i<sel_n;i++)
		colors_list(sel_elements[i], i);
	printf("\n%s%sEnter 'q' to quit.\n", NC, default_color);
	fputs("Elements to be deselected (ex: 1 2 6, or *)? ", stdout);
...
Do you want to create it anyway? (y/n) ");
			...
			if (strcmp(answer, "y") == 0 || strcmp(answer, "Y") == 0);
				...
				fprintf(stderr, "%s: link: invalid answer\n", PROGRAM_NAME);
				free(answer);
				free(source);
				return;
			}
...
void copy_function(char **comm)
{
	...
	if (args_n == 0 || (args_n <= 2 && strcmp(comm[1], "sel") == 0)) {
		if (sel_n > 0) {
			...
			save_sel();
			if (!comm_fail && cd_lists_on_the_fly == 1) { /*No errors. In case of error, don't refresh the files list 
...
//this is my humble version of 'ls -hl' command plus most of the info from 'stat' command.
{
	...
	if (file_attrib.st_mode & S_IFDIR) { fputs("Total size: ", stdout); dir_size(filename); }
	else printf("Size: %s\n", (size_type) ? size_type : "??");
...
//			free_dirlist();
	...
	else fprintf(stderr, "hidden: %s: Invalid argument\nUsage: hidden [on, off]\n", comm[1]);		
}
...
//		else
			strncat(full_comm, comm[i], PATH_MAX-strlen(full_comm));
	}
	char *date=get_date();
	char full_log[PATH_MAX+100]="";
	snprintf(full_log, PATH_MAX+100, "[%s] %s:%s:%s\n", date, user, path, full_comm);
	free(date);
	FILE *log_fp;
...
void backup_function(char **comm, char *file)
{
	...
	else if (args_n == 0) { //if no path has been passed to the function...
		update_path(BK_DIR);
		if (cd_lists_on_the_fly == 0) return;
		while (files--) free(dirlist[files]);
//		free_dirlist();
...
but will also show its content.\n", yellow, NC, default_color, PROGRAM_NAME);
	printf("%s  b, back%s%s [h, hist] [clear] [ELN]: Unlike 'cd ..', which will send you to the parent \
directory of the current directory, this command (with no argument) will send you back to the \
previously visited directory. CliFM keeps a record of all the visited directories. You can see \
this list by typing 'b hist', 'b h' or 'bh', and you can access any element in this list by simply \
passing the corresponding ELN in this list to the 'back' command. Example:\n\
	[user@hostname:S] ~ $ b hist\n\
	1 /home/user\n\
	2 /etc\n\
	3 /proc\n\
	[user@hostname:S] ~ $ b 3\n\
	[user@hostname:S] /proc $ \n\
  Note: the line printed in green indicates the current position of the back function in the \
...
Selection Box. 'Sel' accepts individual elements, range of elements, say 1-6, \
...
as argument, it will delete all the entries in the history file.\n", yellow, NC, default_color);
	printf("You can use the exclamation character (!) to perform some history commands:\n\
  !!: Expand to the last command.\n\
  !n: Expand to command with history number \"n\".\n\
  !-n: Expand to command that was \"n\" number of commands before the last command in history.\n");
...
%s  C-y%s: Toggle system-shell on/off\n\
...
operations you may expect from any other FM. However, the most salient features of %s are: \
\n  a) The use of single character commands and numbers, instead of filenames. Type 'o 12', for \
instance, to open a file with your default text editor or to change to the desired directory.\
\n  b) Bookmarks: bookmark your favorite directories, and even files, to get easy access to them. \
Example: type 'bm' to open the bookmarks screen and then simply type a number or a hotkey to \
access the desired bookmark. \
\n  c) The Selection Box allows you to drop files and directories from different parts of your \
filesystem and then operate on them with just one command. \
\n  d) The back function keeps track of all the paths visited by you so that you can go back \
to any of them by just typing 'b' followed by the corresponding number.\
\nAll of these features (described in more detail below) are aimed to make easier and faster \
to move through and to operate on your files.\n\
Because inspired in Arch Linux and its KISS principle, %s is fundamentally aimed to be \
lightweight, fast, and simple. On Arch's notion of simplcity see: \
...
\n -l, --no-list-on-the-fly\t 'cd' works as the built-in 'cd'\
\n -L, --list-on-the-fly\t\t 'cd' lists files on the fly (default)\
\n -p, --path /starting/path\t use /starting/path as CliFM starting path\
\n -v, --version\t\t\t show version details and exit\n");
	printf("\nCommand line arguments take precedence over both the configuration file and the \
default options, whereas the configuration file takes precedence only over default options.\n");
	printf("\n%sColor codes:%s%s\n\n", white, NC, default_color);
	color_codes();
	printf("%sCommands:\n%sNote: ELN = Element List Number. Example: In the line \"12 openbox\", \
12 is the ELN corresponding to openbox.\n%s", white, NC, default_color);
	list_commands();
...
the -s, --system flag\n\nIt's important to note that, due to %s own parsing method, some \
external commands, especially complex ones, might not work as expected (mostly showing no \
output at all). In these cases, ver few to be sure, you should try running the same \
command preceded by a semicolon (;), since in this way CliFM parsing will be completely \
by-passed and the whole string sent to the system shell.\n\n", PROGRAM_NAME);
...
can also permanently set here some custom variables, ex: 'dir=\"/path/to/folder\"'. \
This variable may be used as a shorcut to that folder, for instance: 'cd $dir'. Custom variables \
could also be temporarily defined via the command prompt: Ex: user@hostname ~ $ var=\"This \
...
working directory) that: 1) there are files in the Selection Box; and 2) you are using %s's \
shell.\n", green, d_cyan, NC, PROGRAM_NAME);
...
void license()
{
printf("Copyright (c) 2018, L. Abramovich\n\n\
This program is free software; you can redistribute it and/or modify \
it under the terms of the GNU General Public License (version 2) as \
published by the Free Software Foundation.\n\n\
...
void splash(void)
{
	...
	"\n                       Press Enter key to continue... ", d_cyan, NC);
	getchar();
}
