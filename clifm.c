...
/* Compile as follows:
 * $ gcc -O2 -march=native -s -fstack-protector-strong -lreadline -lcap -o 
 * clifm clifm.c
...
#define VERSION "0.11.6-2"
#define AUTHOR "L. Abramovich"
#define CONTACT "johndoe.arch@outlook.com"
#define DATE "Dec 5, 2019"
...
/* For TCC compilation: without this variable, TCC complains that 
__dso_handle is an undefined symbol and won't compile */
void* __dso_handle;
...
directory. Using '%s' as an emergency home\n"), PROGRAM_NAME, __func__, 
	 ...
	 * line, that is, as external argument), set the default (user's home) */
	if (!path) {
...
/* Take the command entered by the user and call the corresponding function. */
{
	...
	|| strcmp(comm[0], "m") == 0 || strcmp(comm[0], "v") == 0) {
		...
		copy_function(comm);
		/* If 'mv sel' deselect everything, since sel files are not there 
		 * anymore */
		if (is_sel && strcmp(comm[0], "mv") == 0) {
