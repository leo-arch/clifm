...
exec_cmd(char **comm)
{
	...
	|| comm[0][1] == 'l') && !comm[0][2]))) {
		exit_code = dirjump(comm, NO_SUG_JUMP);
		return exit_code;
	}
	...
	|| strcmp(comm[0], "md") == 0 || strcmp(comm[0], "le") == 0)) {
		...
		if ((((*comm[0] == 'c' || *comm[0] == 'l' || *comm[0] == 'm'
		|| *comm[0] == 'r' || *comm[0] == 't') && !comm[0][1]) || (*comm[0] == 'm'
		&& comm[0][1] == 'd' && !comm[0][2]) || (*comm[0] == 'l'
		&& comm[0][1] == 'e' && !comm[0][2])) && (comm[1] && *comm[1] == '-'
