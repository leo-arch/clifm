...
int
rl_cycle_group_dirs(int count, int key)
{
	UNUSED(count); UNUSED(key);
	if (kbind_busy == 1)
		return FUNC_SUCCESS;
	...
	conf.group_dirs++;
	if (conf.group_dirs > 2)
		conf.group_dirs = GROUP_DIRS_FALSE; // 0
	...
	print_reload_msg(NULL, NULL, _("Group directories: %s\n"),
		conf.group_dirs == GROUP_DIRS_FIRST ? _("first")
		: (conf.group_dirs == GROUP_DIRS_LAST ? _("last") : ("false")));
	xrl_reset_line_state();
	return FUNC_SUCCESS;
}
...
int
rl_toggle_ignore_case(int count, int key)
{
	UNUSED(count); UNUSED(key);
	...
	conf.ignore_case = !conf.ignore_case;
	...
	print_reload_msg(NULL, NULL, _("Ignore case: %s\n"),
		conf.ignore_case == 1 ? _("on") : _("off"));
	xrl_reset_line_state();
	return exit_status;
}
...
rl_toggle_disk_usage(int count, int key)
{
	...
	static int dsort = DEF_SORT, dlong = DEF_LONG_VIEW,
		ddirsize = DEF_FULL_DIR_SIZE, dgd = DEF_GROUP_DIRS;
...
set_keybinds_from_file(void)
{
	struct keyfuncs_t keys[] = {
		...
		{"group-dirs", rl_cycle_group_dirs},
...
set_default_keybinds(void)
{
	struct keyfuncs_t keys[] = {
		...
		{"\\M-y", rl_toggle_light_mode}, {"\\M-g", rl_cycle_group_dirs},
