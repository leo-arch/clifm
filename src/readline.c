...
#ifndef _NO_TAGS
static struct dirent **tagged_files = NULL;
...
my_rl_getc(FILE *stream)
{
	...
	while (1) {
		if (mouse_pop_pending_byte(&c) == 1)
			result = (int)sizeof(unsigned char);
		else {
			const int fd = fileno(stream);
			const int wait_ms = mouse_pending_single_wait_ms();
			if (wait_ms >= 0) {
			...
			result = (int)read(fd, &c, sizeof(unsigned char)); /* flawfinder: ignore */
		}

		if (result == sizeof(unsigned char)) {
			/* Give single-click insertion precedence over typed keys while
			 * still allowing a second click (ESC sequence) to trigger double-click. */
			if (c != KEY_ESC && mouse_pending_single_wait_ms() >= 0)
				mouse_commit_pending_single_click();

			if (c == KEY_ESC) {
				const int mouse_ret = mouse_handle_escape(stream);
...
int_cmds_generator(const char *text, int state)
{
	...
	static const char *const cmd_desc[] = {
		...
		"od      (toggle list-directories-only)",
...
int_z_cmds_generator(const char *text, int state)
{
	...
	static const char *const cmd_desc[] = {
		...
		"zd (toggle list-directories-first)",
		"zf (toggle follow-symlinks)",
		"zh (toggle hidden-files)",
		"zi (toggle icons)",
		"zn (toggle max-filename-len)",
		"zo (toggle list-directories-only)",
...
static char *
gen_comp_cache_file(const char *cmd)
{
	static const char *xdg_cache_home = NULL;
	if (!xdg_cache_home)
		xdg_cache_home = getenv("XDG_CACHE_HOME");

	static char cache_file[PATH_MAX + 1];
	if (xdg_cache_home) {
		snprintf(cache_file, sizeof(cache_file), "%s/%s/completions/%s.clifm",
			xdg_cache_home, PROGRAM_NAME, cmd);
	} else {
...
get_shell_cmd_opts(char *cmd)
{
	*ext_opts[0] = '\0';
	...
	const char *cache_file = gen_comp_cache_file(cmd);

	struct stat a;
	if (stat(cache_file, &a) == -1) {
		/* Comp file does not exist. Try to generate via manpages_comp_gen.py */
		if (gen_shell_cmd_comp(cmd) != FUNC_SUCCESS || stat(cache_file, &a) == -1)
			return FUNC_FAILURE;
	}

	int fd;
	FILE *fp = open_fread(cache_file, &fd);
	if (!fp)
		return FUNC_FAILURE;
