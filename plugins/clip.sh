...
send_to_clip() {
	...
	elif type clip >/dev/nyll 2>&1; then # Cygwin
		clip < "$CLIFM_SELFILE"
	fi
}
...
print_help() {
	...
	printf "\n\x1b[1mUSAGE\x1b[0m\n  %s [e, export] [i, import]\n" "$name"
	printf "\nUse the 'e, export' parameter to send selected files to the system clipboard (as a newline separated list of entries), and the 'i, import' parameter to import files from the system clipboard to CliFM's Selection Box\n"
}
